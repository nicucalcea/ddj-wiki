...
::: {.callout-note}
These notes are mostly inspired from the [Practical AI for (investigative) journalism](https://www.eventbrite.com/e/practical-ai-for-investigative-journalism-tickets-880990695887) sessions.
:::

There are many excellent uses for AI in journalism. You can find some of them on the [Journalist's Toolbox](https://journaliststoolbox.ai/) website.

However, modern generative AIs tends to hallucinate, or say things that are not accurate. For example, here's what [Claude](https://claude.ai/) says when you ask it `Who is Nicu Calcea?`.
...
Clause seems to be very confident and gives a detailed response. However, every single one of the "key facts" on that list is false. I know, because I am [Nicu Calcea](https://nicu.md/), and there is no Romanian politician with the same name.
...
You can see this in action in the [OpenAI Playground](https://platform.openai.com/playground?mode=complete). Copy the last sentence in the text box, make sure you have the `Show probabilities` box ticked on and click `Submit` to let the LLM complete it.
